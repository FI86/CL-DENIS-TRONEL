...
# Nous n'indiquerons pas la bande suivante qui est la tolérance.
...
from tkinter import Tk, Label, Button, Entry, Canvas, END
...
class Resistance:
    def __init__(self):
        ...
        Button(self.root, text="Quitter", command=self.quitter).grid(row=3, sticky="E")
    ...
    def videEntree(self):
        ...
        self.entree.delete(0, END)		        # efface le contenu
    ...
    def arrondiNbrChiffre(self, nb):
        try:
            valeur = int(self.lireEntree())
        except ValueError:
            print("erreur, ce doit etre un entier")
            self.signaleErreur()
